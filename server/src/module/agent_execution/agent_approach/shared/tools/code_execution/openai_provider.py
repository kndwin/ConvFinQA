...
from .provider import CodeExecutionProvider, CodeExecutionSnapshot, CodeExecutionUpdate
...
class OpenAICodeExecutionProvider(CodeExecutionProvider):
    ...
    def normalize_raw_event(self, event: object) -> CodeExecutionUpdate | None:
        ...
        if event_type == "response.output_item.done" and isinstance(
            getattr(raw, "item", None), ResponseCodeInterpreterToolCall
        ):
            item = raw.item
    ...
    def normalize_run_item_event(self, event: object) -> CodeExecutionUpdate | None:
        ...
        if event.name == "tool_called" and isinstance(event.item, ToolCallItem):
            ...
            if not isinstance(raw, ResponseCodeInterpreterToolCall):
                return None
            return self._snapshot(raw, raw.id)
