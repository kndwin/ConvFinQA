...
class ChatSessionApproachSchemaTests(unittest.TestCase):
    ...
    def test_openapi_describes_models_and_rejects_unknown_http_values(self) -> None:
        ...
        self.assertEqual(
            ...
            ["baseline", "baseline-tool", "program-of-thought", "ensemble"],
        )
        self.assertEqual(
...
class AgentExecutionServiceTests(unittest.TestCase):
    def _run(self, approach, model=OpenAIModel.GPT_5_6_SOL):
        ...
        with (
            patch(
                "src.module.agent_execution.execution.direct.agents_execution.Runner.run_streamed",
                side_effect=run_streamed,
            ) as runner,
            patch(
                "src.module.agent_execution.execution.direct.agents_execution.OpenAIProvider"
            ) as provider,
        ):
