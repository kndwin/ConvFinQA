...
import { ChatGroupPanel } from "../chat-group/chat-group.ui";
...
export function DatasetConversationDetailPage() {
  ...
  const onBack = () =>
    void navigate({
      to: "/",
      search: { q: "", split: "all", page: 1, tags: [] },
    });
  ...
  return (
    <DatasetConversationDetail
      state={{
        status: "ready",
        item,
        chat: <ChatGroupPanel datasetId={item.id!} />,
        activeTab: snapshot.matches({ tab: "record" })
          ? "record"
          : "chat-sessions",
        onTabChange: (tab) =>
          send({
