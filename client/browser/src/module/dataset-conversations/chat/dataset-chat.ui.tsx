...
} from "../chat-session-groups/chat-session-groups.query";
...
const agentApproachLabels: Record<AgentApproach, string> = {
  ...
  ensemble: "Ensemble (3 candidates)",
};
const ensembleCandidateApproaches = ["baseline", "baseline-tool", "program-of-thought"] as const;
type EnsembleCandidate = (typeof ensembleCandidateApproaches)[number];
const defaultEnsembleCandidates: EnsembleCandidate[] = [...ensembleCandidateApproaches];
type GroupConfig = {
  id: string;
  agent_approach: AgentApproach;
  model: OpenAIModel;
  ensemble_candidates?: EnsembleCandidate[];
};
  ...
  const [groupConfigs, setGroupConfigs] = useState<GroupConfig[]>(() => [
    {
      ...
      ensemble_candidates: [...defaultEnsembleCandidates],
    },
    {
      id: crypto.randomUUID(),
      agent_approach: "baseline" as AgentApproach,
              ...
              <div className="space-y-2">
                {groupConfigs.map((config, index) => (
                  <div className="space-y-1 rounded border p-2" key={config.id}>
                              ...
                              i === index
                                ? {
                                    ...
                                    ensemble_candidates:
                                      value === "ensemble"
                                        ? [
                                            ...(item.ensemble_candidates ??
                                              defaultEnsembleCandidates),
                                          ]
                                        : undefined,
                                  }
                                : item,
                    ...
                    {config.agent_approach === "ensemble" && (
                      <fieldset className="space-y-2 rounded border bg-muted/20 p-2">
                        <legend className="px-1 text-xs font-medium">Candidates (in order)</legend>
                        <p className="text-xs text-muted-foreground">
                          Select two or three approaches. They run concurrently before review.
                        </p>
                        {ensembleCandidateApproaches.map((candidate) => {
                          const selected = (config.ensemble_candidates ?? []).includes(candidate);
                          return (
                            <label className="flex items-center gap-2 text-sm" key={candidate}>
                              <input
                                type="checkbox"
                                checked={selected}
                                onChange={() =>
                                  setGroupConfigs((items) =>
                                    items.map((item, i) => {
                                      ...
                                      const current =
                                        item.ensemble_candidates ?? defaultEnsembleCandidates;
                                      if (selected && current.length <= 2) return item;
                                      const next = ensembleCandidateApproaches.filter((value) =>
                                        selected
                                          ? value !== candidate && current.includes(value)
                                          : value === candidate || current.includes(value),
                                      );
                                      return { ...item, ensemble_candidates: next };
                                    }),
                                  )
                                }
                                aria-describedby={`candidate-help-${config.id}`}
                              />
                              {agentApproachLabels[candidate]}
                            </label>
                          );
                        })}
                        <p
                          id={`candidate-help-${config.id}`}
                          className="text-xs text-muted-foreground"
                        >
                          {(config.ensemble_candidates ?? []).length} of 3 selected; at least 2
                          required.
                        </p>
                      </fieldset>
                    )}
                    ...
                    onClick={() =>
                      setGroupConfigs((items) => [
                        ...items,
                        {
                          ...
                          id: crypto.randomUUID(),
                          ensemble_candidates: [
                            ...(items[items.length - 1].ensemble_candidates ??
                              defaultEnsembleCandidates),
                          ],
                        },
                      ])
                ...
                <Button
                  ...
                  disabled={
                    ...
                    groupConfigs.some(
                      (config) =>
                        config.agent_approach === "ensemble" &&
                        ((config.ensemble_candidates ?? []).length < 2 ||
                          new Set(config.ensemble_candidates).size !==
                            config.ensemble_candidates?.length),
                    )
                  }
                  onClick={() =>
                    createGroup
                      .mutateAsync({
                        ...
                        sessions: groupConfigs.map(
                          ({ agent_approach, model, ensemble_candidates }) =>
                            Object.assign(
                              { agent_approach, model, tags: [] },
                              agent_approach === "ensemble"
                                ? {
                                    ensemble_candidates: [
                                      ...(ensemble_candidates ?? defaultEnsembleCandidates),
                                    ],
...
const candidateIdPattern = /^ensemble-candidate:.+:(baseline|baseline-tool|program-of-thought)$/;
// TanStack useChat exposes AG-UI text messages but not CustomEvents. The server's
// failure text therefore remains visible in this diagnostic card; event-level
// started/completed/failed badges would require replacing the transport.
function candidateApproachFromId(id: string): EnsembleCandidate | null {
  const match = candidateIdPattern.exec(id);
  return match && ensembleCandidateApproaches.includes(match[1] as EnsembleCandidate)
    ? (match[1] as EnsembleCandidate)
    : null;
}

function ensembleRunId(messages: readonly { id: string }[]) {
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    const id = messages[index]?.id;
    if (id?.startsWith("ensemble-assistant:")) return id.slice("ensemble-assistant:".length);
    const candidate = id?.match(
      /^ensemble-candidate:(.+):(baseline|baseline-tool|program-of-thought)$/,
    );
...
>(({ datasetId, sessionId, initialMessages, agentApproach, onDone, onError }, ref) => {
  ...
  const candidateMessages = chat.messages.filter(
    (message) => candidateApproachFromId(message.id) !== null,
  );
  const visibleMessages = chat.messages.filter(
    (message) => candidateApproachFromId(message.id) === null && hasVisibleTextPart(message),
  );
  const cancel = async () => {
    const runId = ensembleRunId(chat.messages);
    if (agentApproach === "ensemble" && runId) {
      try {
        await fetch(`${chatRunUrl(datasetId, sessionId)}/${encodeURIComponent(runId)}`, {
          method: "DELETE",
        });
      } finally {
        chat.stop();
        ...
        <MessageScroller className="min-h-0 flex-1">
          <MessageScrollerViewport>
            <MessageScrollerContent className="pr-2" aria-busy={chat.isLoading}>
              {visibleMessages.length === 0 && candidateMessages.length === 0 && (
              ...
              {candidateMessages.map((m) => {
                const approach = candidateApproachFromId(m.id);
                if (!approach) return null;
                const candidateRunning = chat.isLoading;
                return (
                  <MessageScrollerItem key={m.id} messageId={m.id}>
                    <details
                      className="mr-8 overflow-hidden rounded-lg border border-border/70 bg-muted/10"
                      open
                    >
                      <summary
                        aria-label={`${candidateRunning ? "Running" : "Completed"} candidate: ${agentApproachLabels[approach]}`}
                        className="cursor-pointer list-none outline-none ring-offset-background focus-visible:ring-2 focus-visible:ring-ring [&::-webkit-details-marker]:hidden"
                      >
                        <Marker className="rounded-none border-0 bg-muted/30 px-3 py-2 hover:bg-muted/50">
                          <MarkerIcon>
                            {candidateRunning ? (
                          ...
                          <MarkerContent>
                            <span className="font-medium text-foreground">
                              Diagnostic candidate: {agentApproachLabels[approach]}
                            </span>
                            <span className="ml-2">
                              {candidateRunning ? "Running…" : "Completed"}
                            </span>
                          </MarkerContent>
                        </Marker>
                      </summary>
                      <div className="space-y-2 border-t border-border/60 p-3">
                        ...
                        {!hasVisibleTextPart(m) && candidateRunning && (
              ...
              {chat.isLoading && (
                <MessageScrollerItem messageId="streaming-status">
                  ...
                  <Marker role="status" className="px-3" aria-live="polite">
                    ...
                    <MarkerContent>
                      {agentApproach === "ensemble"
                        ? "Ensemble candidates and reviewer are replying…"
                        : "Agent is replying…"}
                    </MarkerContent>
                  </Marker>
                </MessageScrollerItem>
